...
class Window(QDialog):
    ...
    def updateGraph(self, outputDict):
        ...
        previousFrame = getattr(self, '_lastAssociationFrame', None)
        self._lastAssociationFrame = self.frameNum
        if previousFrame is None or self.frameNum != previousFrame + 1:
        ...
        if (trackIndexs is not None):
            if (self.configType.currentText() == DEMO_NAME_3DPC or self.configType.currentText() == DEMO_NAME_VITALS):
                if not self.previousClouds:
                    pass
                elif (self.previousClouds[len(self.previousClouds) - 1].shape[0] != trackIndexs.shape[0]):
        ...
        if (self.graphFin):
            ...
            # OpenGL items must be updated on the GUI thread, never during paint.
            try:
                self.get_thread.run()
            finally:
                self.graphFin = 1
...
# TODO select the cfgfile automatically based on the profile.
    ...
    def startApp(self):
        if self.configType.currentText() == DEMO_NAME_VITALS and (
                'maxTracks' not in self.profile or not self.vitalsPatientData):
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.warning(self, 'Configuration required',
                'Select Configuration and load the matching Vital Signs .cfg file first. '
