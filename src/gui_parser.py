...
class uartParser():
    ...
    def readAndParseUart(self):
        ...
        if not hasattr(self, '_rxBuffer'):
            self._rxBuffer = bytearray()
        deadline = time.monotonic() + 0.4
        while True:
            pos = self._rxBuffer.find(magic)
            if pos < 0:
                self._rxBuffer[:] = self._rxBuffer[-7:]
            else:
                del self._rxBuffer[:pos]
                if len(self._rxBuffer) >= 16:
                    length = int.from_bytes(self._rxBuffer[12:16], 'little')
                    if length < 40 or length > 1048576:
                        del self._rxBuffer[0]
                        continue
                    if len(self._rxBuffer) >= length:
                        frame = bytes(self._rxBuffer[:length])
                        if not validateStandardFrame(frame):
                            # Keep subsequent bytes: the next frame may already be here.
                            del self._rxBuffer[0]
                            continue
                        del self._rxBuffer[:length]
                        return parseStandardFrame(frame)
            if time.monotonic() >= deadline:
                return None
            chunk = self.dataCom.read(min(max(self.dataCom.in_waiting, 1), 65536))
            if not chunk:
                return None
            self._rxBuffer.extend(chunk)
