...
def validateStandardFrame(data):
    # Validate the complete TLV layout before dispatching any payload.
    if len(data) < 40 or data[:8] != bytes.fromhex('0201040306050807'):
        return False
    h = struct.unpack_from('<Q8I', data)
    if h[2] != len(data) or h[6] > 65536 or h[7] > (len(data)-40)//8:
        return False
    offset = 40
    for _ in range(h[7]):
        if offset + 8 > len(data):
            return False
        typ, size = struct.unpack_from('<II', data, offset)
        offset += 8
        if size > len(data) - offset:
            return False
        if typ == 1040 and size != 136:
            return False
        if typ == 1010 and size % 112:
            return False
        if typ == 1020 and (size < 20 or (size-20) % 8):
            return False
        offset += size
    return len(data) - offset < 32

def parseStandardFrame(frameData):
    if not validateStandardFrame(frameData):
        return {'error': 2}
    headerStruct = '<Q8I'
